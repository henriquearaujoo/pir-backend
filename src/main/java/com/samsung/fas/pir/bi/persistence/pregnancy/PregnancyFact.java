...
@Entity
@Table(schema = "fas_warehouse", indexes = @Index(name = "idx_pregnancy_fact", unique = true, columnList = "date_id, agent_id, measure_id, social_id, family_service_id, family_social_id, community_location_id, community_service_id, community_social_id"))
public class PregnancyFact extends BIFactBase {
	@Getter
	@Setter
	...
	@Setter
	@ManyToOne(cascade = CascadeType.PERSIST)
	@JoinColumn(foreignKey = @ForeignKey(name = "fk_measure_dimension"))
	private				PregnancyMeasureDimension				measure;
	// endregion
