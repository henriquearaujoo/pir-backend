...
@Entity
@Table(schema = "fas_warehouse", indexes = @Index(name = "idx_answer_fact", unique = true, columnList = "date_id, answer_id, question_id, agent_id, data_id, pregnancy_measure_id, pregnant_social_id, child_social_id, family_service_id, family_social_id, community_location_id, community_service_id, community_social_id"))
public class AnswerFact extends BIFactBase {
	@Getter
	@Setter
	...
	@Setter
	@ManyToOne(cascade = CascadeType.PERSIST)
	@JoinColumn(foreignKey = @ForeignKey(name = "fk_answer_dimension"))
	private 			AnswerDimension 						answer;

	@Getter
	@Setter
	@ManyToOne(cascade = CascadeType.PERSIST)
	@JoinColumn(foreignKey = @ForeignKey(name = "fk_question_dimension"))
	private 			QuestionDimension 						question;
