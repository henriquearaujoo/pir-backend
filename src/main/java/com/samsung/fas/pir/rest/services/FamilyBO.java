...
public class FamilyBO extends BaseBO<Family, FamilyDAO, FamilyDTO, Long> {
	...
	public FamilyDTO save(FamilyDTO create, Device device, UserDetails account) {
		...
		return family != null? new FamilyDTO(getDao().save(setupFamily(family, community, ((Account) account).getUser())), device, true) : new FamilyDTO(getDao().save(setupFamily(model, community, ((Account) account).getUser())), device, true);
	}
	...
	public FamilyDTO update(FamilyDTO update, Device device, UserDetails account) {
		...
		return family != null? new FamilyDTO(getDao().save(setupFamily(family, model, community, ((Account) account).getUser(), device)), device, true) : new FamilyDTO(getDao().save(setupFamily(model, community, ((Account) account).getUser())), device, true);
	}
	...
	Family setupFamily(Family model, Community community, User agent) {
		model.setCommunity(community);
		model.setChildren(setupChild(model, model.getChildren(), agent));
		model.setPregnant(setupPregnant(model, model.getPregnant(), agent));
		return model;
	}

	Family setupFamily(Family family, Family model, Community community, User agent, Device device) {
		getMapper().map(model, family);
		family.setPregnant(setupPregnant(family, model.getPregnant(), agent));
		family.setChildren(setupChild(family, model.getChildren(), agent));
		return family;
	}

	private Collection<Pregnant> setupPregnant(Family mother, Collection<Pregnant> collection, User agent) {
...
//				return getPregnancyBO().setupPregnancy(item, mother, agent);
	...
	private Collection<Child> setupChild(Family family, Collection<Child> collection, User agent) {
		Collection<UUID>		modelIDs		= collection.stream().map(Base::getUuid).collect(Collectors.toList());
		return collection.stream().map(item -> {
			UUID		uuid		= modelIDs.stream().filter(id -> item.getUuid() != null && id != null && id.compareTo(item.getUuid()) == 0).findAny().orElse(null);
