...
public class GreetingsController extends BController<Greetings, CRUGreetingsDTO, GreetingsDAO> {
	...
	@ResponseBody
	public ResponseEntity<Collection<CRUGreetingsDTO>> search(@QuerydslPredicate(root = Greetings.class) Predicate predicate, @ApiIgnore @AuthenticationPrincipal UserDetails details) {
		return ResponseEntity.ok(service.findAll(predicate, details));
	}

	@RequestMapping(method=RequestMethod.GET, value="/search/page")
	@ResponseBody
	public ResponseEntity<Page<CRUGreetingsDTO>> search(@QuerydslPredicate(root = Greetings.class) Predicate predicate, Pageable pageable, @ApiIgnore @AuthenticationPrincipal UserDetails details) {
