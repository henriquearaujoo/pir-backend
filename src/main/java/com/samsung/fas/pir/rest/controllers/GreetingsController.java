...
public class GreetingsController extends BController<GreetingsBO, GreetingsDTO> {
	...
	public ResponseEntity<Collection<GreetingsDTO>> search(@QuerydslPredicate(root = Greetings.class) Predicate predicate, Device device, @ApiIgnore @AuthenticationPrincipal UserDetails details) {
		return ResponseEntity.ok(getService().findAll(predicate, device, details));
	}

	@RequestMapping(method= RequestMethod.GET, value="/search/page")
	public ResponseEntity<Page<GreetingsDTO>> search(@QuerydslPredicate(root = Greetings.class) Predicate predicate, Pageable pageable, Device device, @ApiIgnore @AuthenticationPrincipal UserDetails details) {
